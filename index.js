    ...
    try {
        ...
        switch (command) {
            ...
            case 'ytsearch':
                ...
                try {
                    ...
                    hasil.forEach(async(res) => {
                        ...
                        caption += `\`❖ Di Publikasikan :\` *${res.published}*\n`
                        caption += `\`❖ Penonton    :\` *${res.views}*\n`
            ...
            case 'ytmp3':
                ...
                caption += `\`❖ Pengunggah :\` *${result.uploader}*\n`
                caption += `\`❖ Durasi :\` *${result.duration}*\n`
                caption += `\`❖ Penonton     :\` *${result.view}*\n`
                caption += `\`❖ Suka     :\` *${result.like}*\n`
                caption += `\`❖ Tidak Suka  :\` *${result.dislike}*\n`
                caption += `\`❖ Ukuran     :\` *${result.link[3].size}*`
            ...
            case 'spotify':
                ...
                result = result.result
                caption = `\`❖ Judul      :\` *${result.title}*\n`
                caption += `\`❖ Artis    :\` *${result.artists}*\n`
                caption += `\`❖ Durasi   :\` *${result.duration}*\n`
                caption += `\`❖ Popularitas  :\` *${result.popularity}*`
                await lol.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
            ...
            case 'spotifysearch':
                ...
                try {
                    ...
                    hasil.forEach(async(res) => {
                        caption = `\`❖ Judul     :\` *${res.title}*\n`
                        caption += `\`❖ Artis   :\` *${res.artists}*\n`
                        caption += `\`❖ Link      :\`* ${res.link} *\n`
                        caption += `\`❖ Durasj  :\` *${res.duration}*\n`
                        await reply(caption)
                    })
                } catch (e) {
                    help.messageError(lol)
            ...
            case 'character':
                ...
                result = result.result
                text = `Id : ${result.id}\n`
                text += `Nama : ${result.name.full}\n`
                text += `Asli : ${result.name.native}\n`
                text += `Favorit : ${result.favourites}\n`
                text += `Media : \n`
            ...
            case 'manga':
                ...
                text += `Judul : ${result.title.romaji}\n`
                text += `English : ${result.title.english}\n`
                text += `Asli : ${result.title.native}\n`
                text += `Format : ${result.format}\n`
                text += `Chapter : ${result.chapters}\n`
                text += `Volume : ${result.volumes}\n`
                text += `Status : ${result.status}\n`
                text += `Sumber : ${result.source}\n`
                text += `Tanggal Mulai : ${result.startDate.day} - ${result.startDate.month} - ${result.startDate.year}\n`
                text += `Tanggal Selesai : ${result.endDate.day} - ${result.endDate.month} - ${result.endDate.year}\n`
                text += `Genre : ${result.genres.join(", ")}\n`
                text += `Sinonim : ${result.synonyms.join(", ")}\n`
                text += `Skor : ${result.averageScore}%\n`
                text += `Karakter : \n`
            ...
            case 'anime':
                ...
                text += `Durasi : ${result.duration} mins.\n`
                text += `Status : ${result.status}\n`
                text += `Musim : ${result.season}\n`
                text += `Musim Tahun : ${result.seasonYear}\n`
                text += `Sumber : ${result.source}\n`
                text += `Tanggal Memulai : ${result.startDate.day} - ${result.startDate.month} - ${result.startDate.year}\n`
                text += `Tanggal Selesai : ${result.endDate.day} - ${result.endDate.month} - ${result.endDate.year}\n`
                text += `Genre : ${result.genres.join(", ")}\n`
                text += `Sinonim : ${result.synonyms.join(", ")}\n`
                text += `Skor : ${result.averageScore}%\n`
            ...
            case 'wait':
                if (isQuotedImage || isQuotedAnimation || isQuotedVideo || isQuotedDocument) {
                    ...
                    text += `Judul Romaji : ${result.title_romaji}\n`
                    text += `Judul Native : ${result.title_native}\n`
                    text += `Judul English : ${result.title_english}\n`
                    text += `At : ${result.at}\n`
                    text += `Episode : ${result.episode}\n`
                    text += `Kesamaan : ${result.similarity}`
            ...
            case 'kusonime':
                ...
                text += `Jepang : ${result.japanese}\n`
                text += `Genre : ${result.genre}\n`
                text += `Musim : ${result.seasons}\n`
                text += `Produser : ${result.producers}\n`
                text += `Tipe : ${result.type}\n`
                text += `Status : ${result.status}\n`
                text += `Total Episode : ${result.total_episode}\n`
                text += `Skor : ${result.score}\n`
                text += `Durasi : ${result.duration}\n`
                text += `Rilis Dalam : ${result.released_on}\n`
                text += `Deskripsi : ${result.desc}\n`
